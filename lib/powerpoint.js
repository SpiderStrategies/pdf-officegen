...
class Powerpoint extends EventEmitter2 {
  ...
  _convertWithGhostScript (outputDir, pdfFile, options, callback) {
    ...
    const gsPath = '"C:\\Program Files (x86)\\gs\\gs9.20\\bin\\gswin32c.exe"'
    const gsExecutable = os.platform() === 'win32' ? gsPath : 'gs'
    const imgDir = path.resolve(outputDir, 'img')

    const co = options.convertOptions || {}
    const density = co.density ? Math.min(co.density, DPI_DENSITY_MAX) : DPI_DENSITY_DEFAULT
