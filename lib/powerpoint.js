...
let gsExecutable = GS_PATH || 'gs'
let mupdfExecutable = MUPDF_PATH || 'mudraw'
let inkScapeExecutable = INKSCAPE_PATH || 'inkscape'
let imageMagickConvert = IM_PATH || 'convert'
...
class Powerpoint extends EventEmitter2 {
  ...
  /**
   * PDF conversion utilities ('engines') are configured above with defaults for linux
   * binaries on the system path.  This function inspects any env property that may
   * have been set, and changes the default based on operating system if not set.
   * @private
   */
  _setExecutables () {
    const p = os.platform()
  ...
  _spawnInkscapeShell (commands) {
    return new Promise((resolve, reject) => {
      ...
      inkProc.stdout.on('data', d => {
        ...
        if (_.startsWith(msg, 'Bitmap saved')) {
