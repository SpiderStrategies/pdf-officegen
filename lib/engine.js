...
class Engine extends EventEmitter2 {
  ...
  convert (outputDir, pdfFiles) {
    ...
    if (this._engine === 'inkscape') {
      convertPromise = this._convertWithInkscape(outputDir, pdfFiles, this.options)
    } else if (this._engine === 'mupdf') {
      convertPromise = this._convertWithMuPDF(outputDir, pdfFiles, this.options)
    } else {
      convertPromise = this._convertWithGhostScript(outputDir, pdfFiles, this.options)
    }
