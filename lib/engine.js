...
class Engine extends EventEmitter2 {
  ...
  _convertWithMuPDF (outputDir, pdfFiles) {
    ...
    let tasks = pdfFiles.map((pdfPath, pdfIndex) => {
      return new Promise((resolve) => {
        const imgPrefix = `img-${pdfIndex}-`
        const processFile = mupdfExecutable + (isWindowsPlatform(os.platform()) ? ' draw' : '')
        const args = [`-r ${co.density}`, `-o ${imgPrefix}%d.png`, pdfPath]
        const muStart = nowInMillis()
        execFile(processFile, args, {cwd: this._imgDir}, (err, stdout, stderr) => {
          this.emit('done.mupdf.convert', { output: `${processFile} ${args.join(' ')}`, time: elapsed(muStart), error: err })
          if (err) {
            errors.push(err)
          }
          resolve()
